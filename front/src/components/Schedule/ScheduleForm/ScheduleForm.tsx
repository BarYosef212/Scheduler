...
const ScheduleForm: React.FC<FormProps> = ({ selectedDate, selectedHour }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    toggle();
    try {
      const res = await scheduleBooking(data as Booking);
      setIsConfirmVisible(true);
      if (!res) setScheduleError('שגיאה בעת הזמנת התור, אנא נסה שנית ');
    } catch (error) {}
  };
  if (isConfirmVisible) {
    if (scheduleError) {
      return <p style={{ color: 'red' }}>{scheduleError}</p>;
    }
    return (
      <p>
